...
@export var groups: Array[BlendTreeGroup] = []
...
func groups_for(p_tree_key: String) -> Array[BlendTreeGroup]:
	var result: Array[BlendTreeGroup] = []
	for group in groups:
...
func group_with_id(p_id: String) -> BlendTreeGroup:
	if p_id.is_empty():
		return null
	for group in groups:
...
func group_containing(p_tree_key: String, node_name: String) -> BlendTreeGroup:
...
func build_index(p_tree_key: String) -> Dictionary:
	...
	var children := {}
	for id in by_id:
		var group: BlendTreeGroup = by_id[id]
...
func parent_of(group: BlendTreeGroup, index: Dictionary = {}) -> BlendTreeGroup:
	if group == null or group.parent_id.is_empty():
		return null
	var parent: BlendTreeGroup = index["by_id"].get(group.parent_id) if index.has("by_id") \
...
func child_groups(group: BlendTreeGroup, index: Dictionary = {}) -> Array[BlendTreeGroup]:
	var children: Array[BlendTreeGroup] = []
	if group == null:
		return children
...
func root_groups(p_tree_key: String) -> Array[BlendTreeGroup]:
	var roots: Array[BlendTreeGroup] = []
	for group in groups_for(p_tree_key):
...
func ancestors_of(group: BlendTreeGroup, index: Dictionary = {}) -> Array[BlendTreeGroup]:
	var chain: Array[BlendTreeGroup] = []
	var seen := {}
...
func descendants_of(group: BlendTreeGroup, index: Dictionary = {}) -> Array[BlendTreeGroup]:
...
func subtree_of(group: BlendTreeGroup) -> Array[BlendTreeGroup]:
	var found: Array[BlendTreeGroup] = []
	if group == null:
		return found
	found.append(group)
...
func subtree_members(group: BlendTreeGroup) -> PackedStringArray:
...
func depth_of(group: BlendTreeGroup, index: Dictionary = {}) -> int:
...
func deepest_depth_of(group: BlendTreeGroup, index: Dictionary = {}) -> int:
	if group == null:
		return -1
...
func is_ancestor_of(maybe_ancestor: BlendTreeGroup, group: BlendTreeGroup) -> bool:
...
func can_reparent(group: BlendTreeGroup, new_parent: BlendTreeGroup) -> bool:
	if group == null:
		return false
	if new_parent == null:
		return true
...
func set_parent(group: BlendTreeGroup, new_parent: BlendTreeGroup) -> bool:
...
func sorted_by_depth(p_tree_key: String, index: Dictionary = {}) -> Array[BlendTreeGroup]:
	var buckets: Array = []
...
func has_rendered_members(group: BlendTreeGroup, index: Dictionary = {}) -> bool:
	if group == null:
		return false
...
func add_group(group: BlendTreeGroup) -> void:
	if group == null:
		return
	for member in group.members:
...
func remove_group(group: BlendTreeGroup) -> void:
	if group == null:
		return
...
func _strip_member(p_tree_key: String, node_name: String, keep: BlendTreeGroup) -> void:
	for group in groups:
...
func _drop_empty() -> void:
	...
	while dropped:
		...
		for i in range(groups.size() - 1, -1, -1):
			var group: BlendTreeGroup = groups[i]
