...
const LOCATOR := preload("res://addons/animation_group_manual/editor_locator.gd")
const LAYOUT := preload("res://addons/animation_group_manual/group_layout.gd")
const TOOLBAR := preload("res://addons/animation_group_manual/group_toolbar.gd")
const DIALOG := preload("res://addons/animation_group_manual/group_edit_dialog.gd")
...
const WIRE_LAYER := preload("res://addons/animation_group_manual/group_wire_layer.gd")
...
var _data: ManualBlendTreeGroupData
var _sidecar_path := ""
var _tree_key := ""
...
## reloaded sidecar hands out new ManualBlendTreeGroup objects, and this outlives one.
##
...
## The forest, resolved once per rebuild. See ManualBlendTreeGroupData.build_index.
var _index := {}
...
func _frame_context(group: ManualBlendTreeGroup, contexts: Dictionary) -> Dictionary:
...
func _note_hidden(group: ManualBlendTreeGroup, context: Dictionary, hidden_now: Dictionary) -> void:
...
func _ensure_frame(group: ManualBlendTreeGroup) -> GraphFrame:
...
func _apply_frame(group: ManualBlendTreeGroup, context: Dictionary, plan: Dictionary) -> void:
	var frame_name := FRAME_PREFIX + group.id
...
func _apply_frame_geometry(group: ManualBlendTreeGroup, context: Dictionary, plan: Dictionary) -> void:
...
func _rewrap_ancestors(group: ManualBlendTreeGroup) -> void:
	var parent := _data.parent_of(group, _index)
	if parent == null:
		return
	var frame_name := FRAME_PREFIX + group.id
...
func _apply_frame_parent(frame: GraphFrame, frame_name: String, group: ManualBlendTreeGroup,
...
func _apply_members(frame: GraphFrame, frame_name: String, group: ManualBlendTreeGroup,
...
func _apply_chevron(frame: GraphFrame, group: ManualBlendTreeGroup) -> void:
	...
	if button == null:
		...
		# ManualBlendTreeGroup objects, and a button holding one would edit a ghost.
...
func _apply_titlebar_drag(bar: HBoxContainer, group: ManualBlendTreeGroup) -> void:
...
func _adopt(tree: AnimationNodeBlendTree, key: String) -> void:
	...
	if not own_path.is_empty() and not own_path.contains("::"):
		path = ManualBlendTreeGroupData.sidecar_path_for(own_path)
		tree_key = ""
	else:
		var context := _owner_context()
		if not String(context[0]).is_empty():
			path = ManualBlendTreeGroupData.sidecar_path_for(String(context[0]))
...
func _load_data(path: String) -> ManualBlendTreeGroupData:
	if not path.is_empty() and ResourceLoader.exists(path):
		var loaded := ResourceLoader.load(path, "", ResourceLoader.CACHE_MODE_REUSE)
		if loaded is ManualBlendTreeGroupData:
			...
			(loaded as ManualBlendTreeGroupData).repair_parents()
			return loaded
		push_warning("Animation Group: %s is not group data; ignoring it." % path)
	return ManualBlendTreeGroupData.new()
...
func _save() -> void:
	if _data == null or _sidecar_path.is_empty():
		return
	_data.format_version = ManualBlendTreeGroupData.FORMAT_VERSION
...
func _group_selection() -> void:
	...
	if parent != null and _data.depth_of(parent) + 1 >= ManualBlendTreeGroupData.MAX_DEPTH:
		push_warning("Animation Group: groups are already nested %d deep; not nesting further."
			% ManualBlendTreeGroupData.MAX_DEPTH)
		return

	var group := ManualBlendTreeGroup.make(_default_title(names), names, _next_tint(parent), _tree_key)
	if parent != null:
...
func _wrap_groups(picked: Array[ManualBlendTreeGroup]) -> void:
	...
	if deepest + 1 >= ManualBlendTreeGroupData.MAX_DEPTH:
	...
	var shared: ManualBlendTreeGroup = _data.group_with_id(parent_id)

	var wrapper := ManualBlendTreeGroup.make(
...
func _enclosing_group(names: PackedStringArray) -> ManualBlendTreeGroup:
...
func _descendant_member_positions(group: ManualBlendTreeGroup) -> Dictionary:
	var positions := {}
...
func _selected_groups() -> Array[ManualBlendTreeGroup]:
...
func _is_selected(group: ManualBlendTreeGroup) -> bool:
...
func _groups_of(names: PackedStringArray) -> Array[ManualBlendTreeGroup]:
	var groups: Array[ManualBlendTreeGroup] = []
	if _data == null:
		return groups
	for node_name in names:
...
func _single_group() -> ManualBlendTreeGroup:
	var picked := _selected_groups()
	if picked.size() == 1:
		return picked[0]
	if not picked.is_empty():
...
func _group_titled(title: String) -> ManualBlendTreeGroup:
...
func _next_tint(parent: ManualBlendTreeGroup = null) -> Color:
...
func _update_buttons() -> void:
	...
	var picked := _selected_groups() if has_target else ([] as Array[ManualBlendTreeGroup])
	...
	if has_target and not picked.is_empty():
		...
		can_group = true
	var too_deep := deepest + 1 >= ManualBlendTreeGroupData.MAX_DEPTH
	var group_tip := tip
...
func _update_drag() -> void:
	...
	var scale := EditorInterface.get_editor_scale()
	var moved_boxes: Array[ManualBlendTreeGroup] = []
